...
12. **Primary Cloud AI Vision with Resilient Offline ML Kit Fallback:** In `menu_scan_sheet.dart` and `ai_inward_sheet.dart`, Gemini Deep AI Vision is STRICTLY the PRIMARY scanning engine. Offline ML Kit OCR must NEVER short-circuit or bypass Gemini Cloud AI. ML Kit runs solely as an offline fallback when network is absent or Gemini fails. ML Kit dish validation strictly rejects punctuation-only and column header words (`Rate`, `360/-|`, etc.).
