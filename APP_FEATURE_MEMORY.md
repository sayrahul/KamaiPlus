...
62. **Batch of 14 targeted fixes — Products, Billing, Settings, Pro upgrade (LOCKED, item 13 excepted):**
    ...
    - **NOT implemented — item 13, UPI WhatsApp links:** the `upi://pay?...` links embedded in WhatsApp share text (8 call sites) don't render as tappable in WhatsApp, which only auto-links `http(s)://`. Fixing this needs an HTTP(S) redirector; wrapping via a public shortener was blocked by this session's safety tooling as an external call carrying payment data. See `DEVELOPMENT_LOG.md`'s Known Open Issues — needs the user's decision on how to proceed.
