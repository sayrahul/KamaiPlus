...
## 12. FLUTTER NATIVE ANDROID ARCHITECTURE & PRODUCTION PARITY MATRIX

> **Note**: Yeh section Next.js PWA (`github.com/sayrahul/kamai`) ke sabhi 24 pages, 14 tables aur 150+ business functions ka **Flutter Native Android (`com.kamaiplus.pos`)** me exact mapping aur production status define karta hai.

### 12.1 Native Android Advantages over WebApp
| Feature Layer | WebApp (Next.js / PWA) | Native Android (Flutter + SQLite) | Advantage |
|---|---|---|---|
| **Database** | IndexedDB via Dexie.js (Browser storage limits, clears on cache purge) | **Local SQLite** via `sqflite` (ACID transactions, persistent, <10ms query) | Zero data loss risk, instant counter billing |
| **Monetary Math** | Float math risks in JS (`0.1 + 0.2`) | **Strict Integer Paise Math** (`MoneyFormatter`) | Zero financial rounding discrepancies |
| **Barcode Scanner** | Camera via `html5-qrcode` (web lag, camera permission reload) | **Native `mobile_scanner`** (hardware accelerated camera, 60fps) | Ultra-fast retail scanning at checkout |
| **Thermal Printing** | Web Bluetooth API (browser dependent, pairing drops) | **Direct Android Bluetooth Socket (ESC/POS)** | Auto-reconnect, instant receipt printing |
| **Soundbox Voice** | Web SpeechSynthesis (requires screen awake, voice lag) | **Native TTS / Soundbox Service** (runs offline) | "₹500 rupaye prapt hue" instant Hindi voice |
| **Offline-First** | ServiceWorker caching (can get stale) | **100% Native Offline Execution** | Works in basements, zero internet needed |

---

### 12.2 Master Feature Parity & Wiring Matrix

| WebApp Route | WebApp Functions & Wiring | Flutter Native Screen | SQLite Tables Affected | Side-Effects & Cross-Connections | Production Status |
|---|---|---|---|---|---|
| **`/` (Dashboard)** | `TodayBusinessPulse`, `handleQuickRestock()`, KPI aggregation | `lib/views/dashboard/home_pulse_tab.dart` | `sales`, `products`, `customers`, `expenses` | Reads today's total sales, orders, net profit, low-stock count. Clicking Quick Actions opens Cash Register or Transactions. | ✅ **PROD READY** |
| **`/billing` (POS)** | `handleCompleteSale()`, `handleBarcodeScanned()`, Cart Hold/Resume | `lib/views/pos/pos_billing_screen.dart`, `pos_checkout_modal.dart` | `sales`, `products`, `customers`, `ledger_transactions` | **ATOMIC TRANSACTION**: 1) Inserts sale, 2) Decrements product stock, 3) If Udhar, increases customer balance & creates ledger entry. | ✅ **CORE READY** (Hold/Resume Tabs to add) |
| **`/products` (Catalog)** | `handleSaveProduct()`, `handleBarcodeScanned()`, category filter | `lib/views/products/products_screen.dart`, `add_product_modal.dart` | `products`, `categories` | Changing price or tax rate directly affects POS billing calculations and barcode studio labels. | ✅ **PROD READY** |
| **`/khata` (Udhar Ledger)** | `handleSaveEntry()`, `handleClearCustomerKhata()`, WhatsApp reminder | `lib/views/khata/khata_screen.dart` | `customers`, `ledger_transactions`, `sales` | Debit/Credit updates customer balance. Settle bill updates sale status to 'settled'. Reminders launch WhatsApp with prefilled message. | ✅ **PROD READY** |
| **Menu Hub** | `MobileMenuCardsModal`, 4 category tool grid | `lib/views/menu/menu_screen.dart` (`MenuScreen.show(context)`) | Read-only | **Strictly a Modal Bottom Sheet**. Opens above current screen with 'X' close button. Pushes any tool screen cleanly. | ✅ **PROD READY & LOCKED** |
| **`/cash-register`** | `handleOpenShift()`, `handleConfirmCloseShift()`, `handleSaveExpense()` | `lib/views/cash_register/cash_register_screen.dart` | `expenses`, `sales` | Reconciles cash sales against expenses. Physical note/coin tally counter (₹2000 to ₹1). Z-Report share on WhatsApp. | ✅ **PROD READY** |
| **`/transactions`** | `handleExportCSV()`, `handleOpenInvoice()`, thermal print | `lib/views/transactions/transactions_screen.dart` | `sales` | Filters by date and payment mode. Invoice detail modal with thermal receipt reprint and WhatsApp PDF share. | ✅ **PROD READY** |
| **`/inventory`** | `ExpiryRadar`, `ReorderAlertsList`, `StockMovementsList` | `lib/views/inventory/inventory_screen.dart` | `products` | Valuation eye toggle, Reorder radar pills, direct inward bill trigger. | ✅ **PROD READY** |
| **`/gst-reports`** | `generateGSTR1Report()`, `handleExportCSV()`, Tally XML | `lib/views/reports/gst_reports_screen.dart` | `sales` | 4-metric tax grid, Table 12 HSN summary table, CA Excel & Tally exports. | ✅ **PROD READY** |
| **`/invoice-designer`** | `INVOICE_THEME_PRESETS`, brand color, header/footer | `lib/views/settings/invoice_themes_screen.dart` | `store_profile` | Live interactive A4 invoice paper card, 7 color palettes, heading choices. | ✅ **PROD READY** |
| **`/purchases`** | `handleRecordPurchase()`, Gemini AI bill scan OCR | `lib/views/purchases/purchases_screen.dart`, `ai_inward_sheet.dart` | `products`, `expenses` | Wholesale Inward Hub, AI Bill OCR review sheet, Restock orders, Supplier payment settlement. | ✅ **PROD READY** |
| **`/customers` (CRM)** | `handleSaveCustomer()`, customer purchase history | `lib/views/customers/customers_screen.dart` | `customers`, `sales` | Directory of customers, contact details, outstanding balance, WhatsApp greeting. | 🟡 **IN PROGRESS** |
| **`/barcode-generator`** | `generateCode128SVG()`, print barcode labels | `lib/views/tools/barcode_studio_screen.dart` | `products` | Barcode label designer for 58mm/thermal adhesive rolls. | 🟡 **IN PROGRESS** |
| **`/growth`** | WhatsApp marketing campaigns, broadcast templates | `lib/views/growth/growth_campaigns_screen.dart` | `customers` | Curated retail marketing templates with WhatsApp launch. | 🟡 **IN PROGRESS** |
| **`/settings`** | Store profile, UPI VPAs, GSTIN, thermal printer config | `lib/views/settings/store_profile_screen.dart` | `store_profile` | Store name, address, GSTIN, FSSAI, multi-bank UPI QR codes, Bluetooth printer pairing. | ✅ **PROD READY** |
| **`/cloud-backup`** | Full JSON backup & restore, cloud sync status | `lib/views/settings/backup_restore_screen.dart` | All SQLite Tables | Offline-first database JSON dump and restore. Background Firestore sync bridge. | ✅ **PROD READY** |

---

### 12.3 Critical Side-Effect Golden Rules for Flutter
1. **POS Checkout Side-Effects (`processPosBill`)**:
   - MUST deduct stock from `products`.
   - If payment is `credit`, MUST update `customers.current_balance_paise` and write to `ledger_transactions`.
   - MUST ensure total is calculated using integer paise (`subtotalPaise - discountPaise`).
2. **Wholesale Purchase Inward Side-Effects**:
   - Increment `products.stock_quantity`.
   - Update `products.purchase_price_paise`.
   - If paid from cash till, add an entry to `expenses`.
3. **Khata Settlement Side-Effects (`settleCustomerSaleBill`)**:
   - Update `sales.status = 'settled'`.
   - Decrement `customers.current_balance_paise`.
   - Insert `ledger_transactions` with `type = 'debit'`.

---

*Document updated with complete Flutter Native Android architecture and production roadmap.*
