...
4. **Check `DEVELOPMENT_LOG.md` Too, Before AND After Every Task:** `APP_FEATURE_MEMORY.md` records *what* was decided; `DEVELOPMENT_LOG.md` records *bugs fixed, their root cause, and how they were verified*. Real example of why this matters: commit `ddc25d5` fixed a cross-vertical product-leak bug and added a test for it; five hours later, commit `dbb5997` — working on an unrelated printer/barcode feature — touched the same file and silently reintroduced the exact same bug, because its test only checked data tagging, not the actual screen-facing query. **Before touching `local_database.dart` or any data-loading code, read `DEVELOPMENT_LOG.md`'s "Known open issues" and recent entries** so you don't rediscover or re-break something already fixed. **After fixing any bug**, add an entry there — root cause (file:line), the fix, and how you verified it — not just "fixed X".
