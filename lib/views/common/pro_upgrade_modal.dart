...
class _ProUpgradeModalState extends State<ProUpgradeModal> {
  ...
  Future<void> _loadProfile() async {
    try {
      final p = await LocalDatabase.instance.ensureFreeTrialGranted();
  ...
  Duration get _trialRemainingDuration {
    if (_profile.proExpiry.isEmpty) return Duration.zero;
    final exp = DateTime.tryParse(_profile.proExpiry.trim());
  ...
  Widget build(BuildContext context) {
    final bool isPaidPro = _profile.isProEffective && (_profile.proPlan == 'annual' || _profile.proPlan == 'monthly');
    if (isPaidPro) {
                                    ...
                                    : Row(
                                        ...
                                        children: [
                                          Text(
                                            _isTrialActive
                                                ? (_appliedDiscountPaise != null
                                                    ? 'Extend Pro Validity • ${MoneyFormatter.formatINR(_finalPricePaise)} $periodText'
                                                    : 'Extend Pro Validity • ₹$priceAmount $periodText')
                                                : (_appliedDiscountPaise != null
  ...
  Widget _buildTrialRewardCard() {
    final dur = _trialRemainingDuration;
            ...
            children: [
              Container(
                ...
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Icon(Icons.celebration_rounded, size: 14, color: Color(0xFF0F172A)),
                    const SizedBox(width: 5),
                    Text(
                      'FREE WELCOME REWARD',
                      style: GoogleFonts.outfit(
            ...
            child: Column(
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    _buildCountdownUnit(days.toString().padLeft(2, '0'), 'DAYS'),
  ...
  Widget _buildCountdownUnit(String value, String label, {bool isSeconds = false}) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: isSeconds ? const Color(0xFFF59E0B) : const Color(0xFF34D399),
              width: 1.2,
            ),
          ),
          child: Text(
            ...
            style: GoogleFonts.jetBrainsMono(
              fontSize: 17,
              fontWeight: FontWeight.w900,
              color: isSeconds ? const Color(0xFFFBBF24) : Colors.white,
            ),
          ),
        ),
  ...
  Widget _buildTrialExpiredCard() {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Icon(Icons.history_rounded, size: 22, color: Color(0xFFF87171)),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                const SizedBox(height: 3),
                Text(
                  'Renew your membership below to reactivate Barcode Studio, Cloud Backup, and Unlimited Billing.',
                  style: GoogleFonts.inter(
                    fontSize: 11,
